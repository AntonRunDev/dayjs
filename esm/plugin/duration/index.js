...
var durationAspNetStyleRegex = /(\d+)?\.?\s?(\d{2}):(\d{2}):?(\d{2})?\.?(\d{1,3})?$/;
var unitToMS = {
...
var Duration = /*#__PURE__*/function () {
  function Duration(input, unit, locale) {
    ...
    if (typeof input === 'string') {
      ...
      var durationAspNetStyle = input.match(durationAspNetStyleRegex);
      ...
      if (durationAspNetStyle) {
        var _properties = durationAspNetStyle.slice(1);

        var _numberD = _properties.map(function (value) {
          return value != null ? Number(value) : 0;
        });

        this.$d.days = _numberD[0];
        this.$d.hours = _numberD[1];
        this.$d.minutes = _numberD[2];
        this.$d.seconds = _numberD[3];
        this.$d.milliseconds = _numberD[4];
        this.calMilliseconds();
        return this;
      }
    }
